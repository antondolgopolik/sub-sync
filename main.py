...
def find_time_convergence(start1, end1, start2, end2):
    st1, et1 = sub_items1[start1].start_time, sub_items1[end1].end_time
    st2, et2 = sub_items2[start2].start_time, sub_items2[end2].end_time
    if et1 <= st2 or et2 <= st1:
        return 0
    total_dur = max(et1, et2) - min(st1, st2)
    if st1 <= st2 and et2 <= et1:
        return (et2 - st2) / total_dur
    if st2 <= st1 and et1 <= et2:
        return (et1 - st1) / total_dur
    return (min(et1, et2) - max(st1, st2)) / total_dur


def find_time_divergence(start1, end1, start2, end2):
    return 1 - find_time_convergence(start1, end1, start2, end2)
...
def find_semantic_divergence(start1, end1, start2, end2):
    len1 = end1 - start1 + 1
    len2 = end2 - start2 + 1
    ...
    divergence = (1 - util.cos_sim(tensor_dp1[start1][len1], tensor_dp2[start2][len2])[0][0]) / 2
    return divergence
...
def find_divergence_metric(start1, end1, start2, end2):
    ...
    semantic_div = find_semantic_divergence(start1, end1, start2, end2)
    assert 0 <= time_div <= 1
    assert 0 <= semantic_div <= 1
    if start1 == 13 and end1 == 14 and start2 == 21 and end2 == 22:
        print('true 0: {} + {} = {}'.format(time_div, semantic_div, time_div * 0.6 + semantic_div * 0.4))
    if start1 == 13 and end1 == 14 and start2 == 21 and end2 == 23:
        print('false 0: {} + {} = {}'.format(time_div, semantic_div, time_div * 0.6 + semantic_div * 0.4))
    return time_div * 0.6 + semantic_div * 0.4
    ...
    for i in range(pos1, min(pos1 + 3, len(sub_items1)) + 1):
        for j in range(pos2, min(pos2 + 3, len(sub_items2)) + 1):
            ...
            if (l1 == 0 and l2 != 1) or (l1 != 1 and l2 == 0):
                continue
            val1 = find_divergence_metric(pos1, i - 1, pos2, j - 1)
...
def sync():
    ...
    while pos1 < len(sub_items1) and pos2 < len(sub_items2):
        ...
        sent1 = construct_sentence(pos1, pos1 + len1 - 1, sub_items1)
        sent2 = construct_sentence(pos2, pos2 + len2 - 1, sub_items2)
        print('-----------------------------------')
        print(sent1 + ' -------> ' + sent2)
        if len1 > 0:
